...
export default class Painter {
    ...
    if (view.css && view.css.right) {
      ...
      if (typeof view.css.left === 'string') {
        ...
        for (let i = 1; i < lefts.length;i++){
          self_sum += this.globalTextWidth[lefts[i]]*1
        }
        x = lefts[0].toPx(true) + self_sum
      }
    } else {
      x = 0;
    }
  ...
  _drawAbsImage(view) {
    ...
    if (view.css && view.css.mode === 'scaleToFill') {
      ...
      if (view.css.self_position == 'left'){
        this.ctx.drawImage(view.url, 0, 0, rWidth, rHeight, -(width / 2), -(height / 2), width, height);
      }else{
